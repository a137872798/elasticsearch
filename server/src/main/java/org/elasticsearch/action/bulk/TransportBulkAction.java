...
public class TransportBulkAction extends HandledTransportAction<BulkRequest, BulkResponse> {
    ...
    public static IndexRequest getIndexWriteRequest(DocWriteRequest docWriteRequest) {
        ...
        if (docWriteRequest instanceof IndexRequest) {
            ...
            // docAsUpsert  是否以doc替代 upsert请求
            indexRequest = updateRequest.docAsUpsert() ? updateRequest.doc() : updateRequest.upsertRequest();
        }
        // 如果是 DeleteReq 返回null
        return indexRequest;
    }
     ...
     * 接收到请求后 转发到该方法处理
     * @param task
     * @param bulkRequest  内部包含了一组索引操作  比如 插入/更新/删除
     * @param listener
     */
    @Override
    protected void doExecute(Task task, BulkRequest bulkRequest, ActionListener<BulkResponse> listener) {
        ...
        for (DocWriteRequest<?> actionRequest : bulkRequest.requests) {
            // 从请求体中 抽取出 indexRequest   deleteReq 是不会获取到indexReq
            IndexRequest indexRequest = getIndexWriteRequest(actionRequest);
            if (indexRequest != null) {
                // Each index request needs to be evaluated, because this method also modifies the IndexRequest
                // index 对象在创建时 是否使用了 V2版本的模板  模板的作用就是为了定义 index下有哪些字段
        ...
        if (needToCheck()) {
            ...
            // 代表所有index都已经存在了 直接执行任务就可以
            if (autoCreateIndices.isEmpty()) {
                executeBulk(task, bulkRequest, startTime, listener, responses, indicesThatCannotBeCreated);
            } else {
                // 首先自动创建index  之后执行bulk 任务
    ...
    static boolean resolvePipelines(final DocWriteRequest<?> originalRequest, final IndexRequest indexRequest,
        ...
        if (indexRequest.isPipelineResolved() == false) {
            ...
            // 可能传入的是别名 就无法直接定位到 indexMetadata
            ...
            // 当通过indexRequest 没法找到索引信息时 尝试使用 originReq去查找
            ...
            // 如果req中本身就携带 pipeline信息 那么它的优先级最高
    ...
    /**
     * 是否需要自动创建索引  false 代表索引存在  true代表可以创建   否则会抛出 indexNotFound
     * @param index
     * @param state
     * @return
     */
    ...
    private final class BulkOperation extends ActionRunnable<BulkResponse> {
        ...
        /**
         * bulk 体现在一次性执行多个 索引相关的操作   (插入/更新/删除)
         */
        @Override
        protected void doRun() {
            final ClusterState clusterState = observer.setAndGetObservedState();
            // 代表当前集群状态不支持写入数据   等待集群状态发生变化 并重试
            ...
            for (int i = 0; i < bulkRequest.requests.size(); i++) {
                ...
                // 无法找到索引 或者索引处于关闭状态 将该req设置到失败列表中 并处理下一个req
                ...
                try {
                    ...
                    switch (docWriteRequest.opType()) {
                        ...
                        case INDEX:
                            ...
                            // 每个req 都会指定写入到某个分片对应的路由上  应该都是写入主分片的路由
                            indexRequest.resolveRouting(metadata);
                            // 填充一些相关信息 比如index的mapping模板(描述包含哪些field)  如果id信息不存在 则会自动创建
                        ...
                        case UPDATE:
                            ...
                            break;
                        case DELETE:
                            // 这2步的流程和TransportUpdateAction.resolveAndValidateRouting 基本是一样的
            ...
            for (int i = 0; i < bulkRequest.requests.size(); i++) {
                ...
                // 获取索引名称 req.index() 可能是别名
                String concreteIndex = concreteIndices.getConcreteIndex(request.index()).getName();
                // 通过 operationRouting 来确定本次操作会写入到哪个分片   req.id() req.routing() 会作为决定shardId的关键信息 通过hash函数计算
                ShardId shardId = clusterService.operationRouting().indexShards(clusterState, concreteIndex, request.id(),
                    request.routing()).shardId();

                // 每个索引操作 按照要写入的shardId来划分
                List<BulkItemRequest> shardRequests = requestsByShard.computeIfAbsent(shardId, shard -> new ArrayList<>());
                shardRequests.add(new BulkItemRequest(i, request));
            }

            // 本次选择的 shardId没有相关的路由信息  先忽略这种情况 应该不会发生
            ...
            // 以shardId 为单位 发起索引请求  每个操作都会先在 primary上执行  后在replica执行
            // 因为在ES集群内 每个节点都与其他节点保持长连接  所以不需要通过primary做转发   索引操作本身可以被任何一个节点接收 通过req信息找到目标shardId后 转发到shardId所在的各个节点上
        ...
        /**
         * 检测索引是否不可用 如果不可用将请求设置成处理失败 并存储到容器中
         * @param request
        ...
        private boolean addFailureIfIndexIsUnavailable(DocWriteRequest<?> request, int idx, final ConcreteIndices concreteIndices,
            ...
            if (cannotCreate != null) {
                // 代表对应索引不存在 直接设置到失败容器中
                addFailure(request, idx, cannotCreate);
                return true;
            }
            // 该容器内存储了 indexName(或者是别名之类的) 与索引的映射关系
            Index concreteIndex = concreteIndices.getConcreteIndex(request.index());
            if (concreteIndex == null) {
                try {
                    // 将req对应的index存储到 concreteIndices 中   避免重复解析
    ...
    private static class ConcreteIndices  {
        ...
        Index resolveIfAbsent(DocWriteRequest<?> request) {
            ...
            if (concreteIndex == null) {
                // 这里会使用 req.index 作为索引名 或者别名之类的去查询结果
    ...
    private void processBulkIndexIngestRequest(Task task, BulkRequest original, ActionListener<BulkResponse> listener) {
        ...
        ingestService.executeBulkRequest(
            ...
            // 当所有req 处理完毕后触发该钩子    originalThread 代表执行pipeline时的原始线程  因为在处理管道时 不确定是否会切换到其他线程
            ...
            // 当某个请求在被 pipeline处理后 变成了null 触发该函数
    ...
    static final class BulkRequestModifier implements Iterator<DocWriteRequest<?>> {
        ...
        /**
         * 只有在pipeline处理过程中 出现失败或者被丢弃的 req 才会生成resp 并存储到列表中
         */
        final List<BulkItemResponse> itemResponses;

        /**
         * 在通过pipeline 处理完后 req会重新设置到这个数组中
         */
        ...
        BulkRequestModifier(BulkRequest bulkRequest) {
            this.bulkRequest = bulkRequest;
            // 请求数量会很多么 因为稀疏位图本身的应用场景就是不连续且min/max相差比较大
        ...
        /**
         * 在所有子req经过pipeline处理后  重新包装成bulkReq
         * @return
         */
        BulkRequest getBulkRequest() {
            ...
            if (itemResponses.isEmpty()) {
                ...
                for (int i = 0; i < requests.size(); i++) {
                    // 只有通过管道处理后 未被丢弃 或者失败的 req 才会加入到新的 bulkReq中
        ...
        /**
         * 标记某个req 在被pipeline处理时 数据被丢弃了 (处理后返回null)
         * @param slot
         */
