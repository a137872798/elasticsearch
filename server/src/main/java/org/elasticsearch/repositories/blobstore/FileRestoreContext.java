...
public abstract class FileRestoreContext {
     ...
     * 开始进行数据恢复
     * @param snapshotFiles 本次快照对应的所有文件
     */
    public void restore(SnapshotFiles snapshotFiles, Store store, ActionListener<Void> listener) {
        ...
        try {
            ...
            try {
                ...
                // 如果之前没有任何该分片的索引文件 会出现该异常 这里进行捕获 并修改成一个empty对象
            ...
            // 如果存储分片的仓库下已经有部分数据了 这里只要找出差异的数据即可
            ...
            try {
                ...
                restoreFiles(filesToRecover, store, ActionListener.wrap(
                    v -> {
                        store.incRef();
                        try {
                            // 此时将索引文件从repository 搬回了shard.store
                            // afterRestore 主要是删除快照无关的其他文件
