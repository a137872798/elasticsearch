...
public interface Repository extends LifecycleComponent {
     ...
     * 获取当前存储对象对应的元数据信息 包含使用的配置 以及当前已经存储了多少次
     */
     ...
     * 首先通过 快照id 定位到某个快照 之后通过indexId 定位到某个具体的索引 这里是获取相关的元数据
     */
     ...
     *                              结束一个快照进程   有leader节点调用 当所有分片都生成快照后触发
     */
     ...
     *                              IndexCommit 对应lucene的segmentN 文件好像
     */
